...
## Feature : Page "Mon Profil" + menu déroulant navbar

---

## FICHIER 1 — `frontend/src/pages/MonProfil.jsx`
...
## FICHIER 2 — `frontend/src/components/Navbar.jsx`

### Remplace le "User area" par un menu déroulant

Ajoute `useRef` et `useEffect` dans les imports React :
```js
import { useState, useRef, useEffect } from 'react'
```

Dans le composant `Navbar`, ajoute :
```js
...
Remplace le bloc `{user ? ... : ...}` dans le User area par :
...
### Menu mobile — ajoute le lien Mon Profil

Dans le menu mobile `{user && ...}`, ajoute avant le bouton Déconnexion :
```jsx
...
## FICHIER 3 — `frontend/src/App.jsx`

Ajoute la route `/mon-profil` :
...
```bash
git add .
git commit -m "feat: page Mon Profil + menu déroulant navbar"
